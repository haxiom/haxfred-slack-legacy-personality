...
var haxfred_slack_legacy_personality = function(haxfred) {
  Haxfred = haxfred;

  // Looks for a file to pull random phrases from
  // If it can't be found, creates one.

  var randomPhrasesPath = Haxfred.config.randomPhrasePath || "./randomPhrases";

  if(!fs.existsSync(randomPhrasesPath)) {
  ...
  var randomPhrases = fs.readFileSync(randomPhrasesPath).toString().trim().split(/\n/);
  ...
  // /* ----- Haxfred Listeners ----- */
  // haxfred.on('slack.privateMsg', '', function(data, deferred) {
  //   var from = data.from;
  //   console.log("from:", from);

  //   //haxfred.slack.say("thanks for thinking of me", from);

  //   deferred.resolve();
  // });

  // haxfred.on('slack.join', '', function(data, deferred) {
  //   var from = data.from;

  //   if (haxfred.slack.client.nick !== from) {

  //       //haxfred.slack.say("Hey there, " + from + " welcome to " + Haxfred.config.channels[0] + "!");
  //   }

  //   deferred.resolve();
  // });

  haxfred.on('slack.directMsg', '', function(data, deferred) {
    var channel = haxfred.slack.getChannelGroupOrDMByID(data.channel);
    channel.send(getRandom(randomPhrases));

    deferred.resolve();
  });
};

module.exports = haxfred_slack_legacy_personality;
